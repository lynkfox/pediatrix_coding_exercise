...
Likewise, I did not perform much type checking internally. For example, the Product/Coin objects have tests that confirm the values provided are specific types, but they perform no actual checking that they are. These tests were made a bit out of habit (I always do a test for type of return in Python because Hints) but with the 4 hour limit decided against worrying overly much for that. Again, a good User Story of "As a vending machine, I need to verify a coins weight is a float, and inform the user if it is not acceptable input" ect ect. (I'd probably convert them to dictionaries/json objects like an API of sorts, and run a validation against them with a known approved json schema)
